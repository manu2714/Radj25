...
zsign<-function(x){
  ifelse(abs(x) > 3.279,1,0) 
}


zsignc<-function(x){
  ifelse(x > 3.279, "Act",ifelse(x < -3.279, "Inh", "Ind"))
}
...
chisq8A50<- llply(transiz8A50,function(x) chisq.test(x)$statistic)
...
resid8A50<- llply(transiz8A50, function(x) chisq.test(x)$stdres)
...
tabla8A50<-table(datosLong8A50$CATEGORY,datosLong8A50$Medida)
tabla8A50p<- as.data.frame(prop.table(tabla8A50,1))
colnames(tabla8A50p)<- c("PATTERN", "DEPENDENCY", "P")
tabla8A50p$CATEGORY<- rep(8,128)
tabla8A50p$MATRIX<- rep("A",128)
tabla8A50p$N<- rep(50,128)
...
resid8A100<- llply(transiz8A100, function(x) chisq.test(x)$stdres)
...
tabla8A100<-table(datosLong8A100$CATEGORY,datosLong8A100$Medida)
tabla8A100p<- as.data.frame(prop.table(tabla8A100,1))
colnames(tabla8A100p)<- c("PATTERN", "DEPENDENCY", "P")
tabla8A100p$CATEGORY<- rep(8,128)
tabla8A100p$MATRIX<- rep("A",128)
tabla8A100p$N<- rep(100,128)
...
chisq8A150<- llply(transiz8A150,function(x) chisq.test(x)$statistic)
...
resid8A150<- llply(transiz8A150, function(x) chisq.test(x)$stdres)
...
datosLong8A150<-pivot_longer(zsig8A150, cols = 1:64,values_to = "Medida", names_pattern = "(.*)_(.*)",
                             names_to = c("N", "CATEGORY"))

datosLong8A150$N <- gl(1,320000, labels = 150)
datosLong8A150$CATEGORY<- rep(nombres8,5000)

tabla8A150<-table(datosLong8A150$CATEGORY,datosLong8A150$Medida)
tabla8A150p<- as.data.frame(prop.table(tabla8A150,1))
colnames(tabla8A150p)<- c("PATTERN", "DEPENDENCY", "P")
tabla8A150p$CATEGORY<- rep(8,128)
tabla8A150p$MATRIX<- rep("A",128)
tabla8A150p$N<- rep(150,128)
...
chisq8B50<- llply(transiz8B50,function(x) chisq.test(x)$statistic)
...
resid8B50<- llply(transiz8B50, function(x) chisq.test(x)$stdres)
...
tabla8B50<-table(datosLong8B50$CATEGORY,datosLong8B50$Medida)
tabla8B50p<- as.data.frame(prop.table(tabla8B50,1))
colnames(tabla8B50p)<- c("PATTERN", "DEPENDENCY", "P")
tabla8B50p$CATEGORY<- rep(8,128)
tabla8B50p$MATRIX<- rep("B",128)
tabla8B50p$N<- rep(50,128)
...
resid8B100<- llply(transiz8B100, function(x) chisq.test(x)$stdres)
...
chisq8B150<- llply(transiz8B150,function(x) chisq.test(x)$statistic)
...
resid8B150<- llply(transiz8B150, function(x) chisq.test(x)$stdres)
...
datosLong8B150<-pivot_longer(zsig8B150, cols = 1:64,values_to = "Medida", names_pattern = "(.*)_(.*)",
                             names_to = c("N", "CATEGORY"))

datosLong8B150$N <- gl(1,320000, labels = 150)
datosLong8B150$CATEGORY<- rep(nombres8,5000)

tabla8B150<-table(datosLong8B150$CATEGORY,datosLong8B150$Medida)
...
chisq8C50<- llply(transiz8C50,function(x) chisq.test(x)$statistic)
...
resid8C50<- llply(transiz8C50, function(x) chisq.test(x)$stdres)
...
tabla8C50<-table(datosLong8C50$CATEGORY,datosLong8C50$Medida)
...
resid8C100<- llply(transiz8C100, function(x) chisq.test(x)$stdres)
...
chisq8C150<- llply(transiz8C150,function(x) chisq.test(x)$statistic)
chi8C150<- llply(chisq8C150, as.vector)
chi28C150<- unlist(chi8C150)
buenos<- which(chi8C150 > qchisq(.95,49))
length(buenos)

transiz8C150<- transiz8C150[buenos]

resid8C150<- llply(transiz8C150, function(x) chisq.test(x)$stdres)
...
# N<-rep(20, 64)
# MATRIX<- rep("A", 64)
# df20<- data.frame(PATTERN,DEPENDENCY,P,CATEGORY,MATRIX,N)
# tabla8A20p <- rbind(tabla8A20p,df20)
# 
# 
# N<-rep(20, 64)
# MATRIX<- rep("B", 64)
# df20<- data.frame(PATTERN,DEPENDENCY,P,CATEGORY,MATRIX,N)
# tabla8B20p <- rbind(tabla8B20p,df20)
# 
# 
# N<-rep(20, 64)
# MATRIX<- rep("C", 64)
# df20<- data.frame(PATTERN,DEPENDENCY,P,CATEGORY,MATRIX,N)
# tabla8C20p <- rbind(tabla8C20p,df20)


N<-rep(50, 64)
MATRIX<- rep("A", 64)
df50<- data.frame(PATTERN,DEPENDENCY,P,CATEGORY,MATRIX,N)
tabla8A50p<- rbind(tabla8A50p,df50)


 N<-rep(50, 64)
 MATRIX<- rep("B", 64)
 df50<- data.frame(PATTERN,DEPENDENCY,P,CATEGORY,MATRIX,N)
 tabla8B50p<- rbind(tabla8B50p,df50)


 N<-rep(100, 64)
 df100<- data.frame(PATTERN,DEPENDENCY,P,CATEGORY,MATRIX,N)
 tabla8A100p<- rbind(tabla8A100p,df100)

 N<-rep(150, 64)
 MATRIX<- rep("A", 64)
 df150<- data.frame(PATTERN,DEPENDENCY,P,CATEGORY,MATRIX,N)
 tabla8A150p<- rbind(tabla8A150p,df150)




tabla8_chi_bon<- rbind(tabla8A50p,tabla8A100p,tabla8A150p,
...
tab8bc1<- subset(tabla8_chi_bon,PATTERN == "BC" & DEPENDENCY == "Act" )
tab8dc1<- subset(tabla8_chi_bon,PATTERN == "DC" & DEPENDENCY == "Act" )
tab8_chi_bon<- rbind(tab8bc1,tab8dc1)
tab8_chi_bon$MODEL<- gl(1,18, labels = "BON-GBL")
tab8_chi_bon$N<- rep(gl(3,1, labels = c("20","50","100","150")),6)
...
tab8ba1<- subset(tabla8_chi_bon,PATTERN == "BA" & DEPENDENCY == "Inh" )
tab8bb1<- subset(tabla8_chi_bon,PATTERN == "BB" & DEPENDENCY == "Inh" )
tab8bd1<- subset(tabla8_chi_bon,PATTERN == "BD" & DEPENDENCY == "Inh" )
tab8be1<- subset(tabla8_chi_bon,PATTERN == "BE" & DEPENDENCY == "Inh" )
tab8bf1<- subset(tabla8_chi_bon,PATTERN == "BF" & DEPENDENCY == "Inh" )
tab8bg1<- subset(tabla8_chi_bon,PATTERN == "BG" & DEPENDENCY == "Inh" )
tab8bh1<- subset(tabla8_chi_bon,PATTERN == "BH" & DEPENDENCY == "Inh" )


tab8da1<- subset(tabla8_chi_bon,PATTERN == "DA" & DEPENDENCY == "Inh" )
tab8db1<- subset(tabla8_chi_bon,PATTERN == "DB" & DEPENDENCY == "Inh" )
tab8dd1<- subset(tabla8_chi_bon,PATTERN == "DD" & DEPENDENCY == "Inh" )
tab8de1<- subset(tabla8_chi_bon,PATTERN == "DE" & DEPENDENCY == "Inh" )
tab8df1<- subset(tabla8_chi_bon,PATTERN == "DF" & DEPENDENCY == "Inh" )
tab8dg1<- subset(tabla8_chi_bon,PATTERN == "DG" & DEPENDENCY == "Inh" )
tab8dh1<- subset(tabla8_chi_bon,PATTERN == "DH" & DEPENDENCY == "Inh" )

tab8_inh_chi_bon<- rbind(tab8ba1,tab8bb1,tab8bd1,tab8be1,tab8bf1,
                     tab8bg1,tab8bh1,tab8da1,tab8db1,tab8dd1,
                     tab8de1,tab8df1,tab8dg1,tab8dh1)

tab8_inh_chi_bon$MODEL<- gl(1,126, labels = "BON-GBL")
tab8_inh_chi_bon$N<- rep(gl(3,1, labels = c("50","100","150")),42)

# Spureous pattern

tab8ac1<- subset(tabla8_chi_bon,PATTERN == "AC" & DEPENDENCY == "Inh" )
tab8cc1<- subset(tabla8_chi_bon,PATTERN == "CC" & DEPENDENCY == "Inh" )

tab8_sp_chi_bon<- rbind(tab8ac1,tab8cc1)

tab8_sp_chi_bon$MODEL<- gl(1,18, labels = "BON-GBL")
tab8_sp_chi_bon$N<- rep(gl(3,1, labels = c("50","100","150")),6)
